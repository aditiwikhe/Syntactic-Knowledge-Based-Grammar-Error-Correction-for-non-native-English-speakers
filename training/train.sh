...
if [ -e $PROCESSED_DIR/bin ]; then
    ...
    $SUBWORD_NMT/apply_bpe.py -c $BPE_MODEL_DIR/bpe_code.trg.dict_bpe8000 < $train_src > $PROCESSED_DIR/train.all.src
    $SUBWORD_NMT/apply_bpe.py -c $BPE_MODEL_DIR/bpe_code.trg.dict_bpe8000 < $train_trg > $PROCESSED_DIR/train.all.trg
    ...
    python get_diff.py $PROCESSED_DIR/train.all src trg >$PROCESSED_DIR/train.annotated.src-trg
    ...
    python get_diff.py $DATA_DIR/train src trg >$PROCESSED_DIR/train.annotated.bert.src-trg
    cut -f1 $PROCESSED_DIR/train.annotated.bert.src-trg >$PROCESSED_DIR/train.bert.src
    #cut -f2 $PROCESSED_DIR/train.annotated.bert.src-trg >$PROCESSED_DIR/train.bert.trg
